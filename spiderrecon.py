...
class SpiderRecon:
    def __init__(self, target, is_email_scan_on, is_phone_number_scan_on, max_pages, output_file_path):
        self.target = target
    ...
    def start(self):
        emails, phones = set(), set()  # fallback in case of interruption before assignment
        try:
            emails, phones = self.scan()
...
def main():
    parser = argparse.ArgumentParser(description="SpiderRecon - Email & Phone Number Crawler")
    ...
    parser.add_argument("-m", "--max_pages", type=int, default=1000, help="Max number of pages to crawl (default: 100).")
